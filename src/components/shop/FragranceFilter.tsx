        ...
        <div className="p-6">
            <div className="flex justify-center space-x-6">
                {categories.map((cat, idx) => (
                    <div key={idx} className="text-center cursor-pointer" onClick={() => setDrawerOpen(true)}>
                        <div className="w-25 h-20 rounded-md">
                            <Image src={cat.imageUrl} alt="pic" width={120} height={150} />
                        </div>
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {items.map((item) => (
                                        <div
                                            ...
                                            className={`relative w-20 h-20 rounded-md overflow-hidden cursor-pointer border transition-all duration-200 ${
                                                selectedFilters[section] === item.slug ? 'border-pink-500' : 'border-transparent'
                                            }`}
                                        >
                                            <Image
                                                ...
                                                layout="fill"
                                                objectFit="cover"
                                                className={`${selectedFilters[section] === item.slug ? 'blur-sm' : ''}`}
                                            />
                                            <div className="absolute bottom-0 left-0 right-0 text-xs text-white bg-black bg-opacity-40 text-center py-1">
                                                {item.slug.replace(/_/g, ' ').replace(/\b\w/g, (char:any) => char.toUpperCase())}
                                            </div>
                                        </div>
                                    ))}
                                </div>
