  ...
  return (
    <div className="min-h-screen bg-white text-black flex flex-col">
      ...
      <div className="max-w-4xl mx-auto w-full px-4 flex gap-4 mb-8">
        {scentCategories.map((cat) => (
          <div key={cat.value} className="flex flex-col items-center flex-1">
            ...
            <span
              className={`mt-1 text-sm font-serif ${
                activeCategory === cat.value
                  ? "text-[#600000] font-semibold"
                  : "text-gray-700"
              }`}
            >
              {cat.label}
      ...
      <div className="max-w-7xl mx-auto w-full px-4 flex items-center justify-between mb-6">
        <span className="text-[#a0001e] font-serif">
          Showing {filteredProducts.length} products
        </span>
        <div className="flex items-center gap-2">
          <button className="w-10 h-10 rounded-full border border-white/60 bg-transparent"></button>
          <select
            className="rounded-full border border-white/60 bg-transparent text-[#a0001e] px-4 py-1 text-base font-serif focus:outline-none"
          ...
          >
            <div className="relative bg-[#f8f5f2] rounded-xl p-6 flex flex-col items-center min-h-[550px] group overflow-hidden">
              ...
              {product.isBestSeller && (
                <span className="absolute top-2 right-2 bg-amber-100 text-amber-800 text-xs px-2 py-1 rounded-sm font-medium z-20">
                  BEST SELLER
                </span>
              )}
              {/* Product Image */}
              <div className="relative w-24 h-32 my-4 z-10">
                <Image
                  src={product.imageUrl || "/images/hero/hero_image.png"}
                  alt={product.name}
                  fill
              ...
              <div className="absolute inset-0 bg-black/40 backdrop-blur-md opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-center items-center z-30 p-10 rounded-lg">
                <div className="w-full flex flex-col items-center">
                  <h2 className="text-3xl md:text-3xl font-serif font-bold text-white mb-2 text-center drop-shadow">
                    {product.name}
                  </h2>
                  <div className="text-xl md:text-lg text-white mb-4 text-center font-serif tracking-wide">
                      ...
                      {product.scentNotes.map((note, idx) => (
                        <div key={idx} className="flex flex-col items-center">
                          <div className="relative w-10 h-10 mb-1">
                            <Image
                              src={
                                note.imageUrl || "/images/hero/hero_image.png"
                              }
                              alt={note.name}
                              fill
                              style={{ objectFit: "contain" }}
                  ...
                  <div className="text-white text-lg font-serif font-semibold mb-1 mt-2">
                    Our{" "}
                  ...
                  <div className="text-white text-base font-serif mb-6">
                    Full Bottle Price{" "}
                    <span className="text-[#e6c789]">₦380</span>
                  </div>
                  <button className="w-full bg-[#a00000] hover:bg-[#b30000] text-white text-xl font-serif rounded-xl py-2 transition-colors mb-2">
                    Add to cart
                  </button>
                </div>
              </div>
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center px-4 gap-8">
          <div className="flex-1 text-white">
            <h2 className="text-2xl md:text-3xl font-serif mb-4">
              OVER 200 CURATED FRAGRANCES. ONE SIGNATURE EXPERIENCE
            </h2>
            <p className="mb-6 text-base md:text-lg">
              The luxury perfume world was built on exclusivity. We&apos;re here
              to break that barrier—without watering anything down.
              <br />
              We stock over 200 of the best designer and niche fragrances in the
              world. From Parfums de Marly to Lattafa, Amouage to Armaf.
              <br />
              <br />
              While most decants arrive in plain plastic vials, ours are a
              luxury experience—with premium packaging, mood-based bundles, and
              unforgettable scent storytelling.
              <br />
              Forvr Murr is more than a perfume shop. It&apos;s a curated world
              of scent, desire, and indulgence.
            </p>
            <button className="bg-[#e6c789] text-[#600000] px-6 py-2 rounded font-medium hover:bg-[#f8f5e2] transition-colors">
              Subscribe
            </button>
          </div>
