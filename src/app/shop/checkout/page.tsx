...
import checkoutService, { CheckoutFormData, ShippingMethod, SavedAddress, CheckoutResponse, PaymentResponse } from '@/services/checkout';
...
const CheckoutPage = () => {
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      ...
      try {
        ...
        const countryCode = formData.shippingAddress.country === 'Nigeria' ? 'NG' : formData.shippingAddress.country; 
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
    ...
    if (name === 'useSameForBilling') {
      const isNowSameAsShipping = checked === undefined ? formData.useSameForBilling : checked;
      setFormData(prev => ({
        ...prev,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 3. Initiate payment
      const paymentResponse = await checkoutService.initiatePayment(orderResponse.id);
  ...
  const shippingCost = selectedShippingMethod ? parseInt(selectedShippingMethod.amount) : 0; // Use .amount instead of .price
          ...
          <div className="flex flex-col md:flex-row items-center justify-between mb-8">
            ...
            <div className="text-sm breadcrumbs">
              <ul className="flex items-center space-x-2">
                ...
                <li className="text-gray-900 before:content-['>'] before:mx-2">Checkout</li>
              </ul>
            </div>
          </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={handleInputChange}
                          className={`w-full p-2 border bg-white border-gray-300 rounded ${(isAuthenticated && user?.email) ? 'bg-gray-100' : ''}`}
                          required
