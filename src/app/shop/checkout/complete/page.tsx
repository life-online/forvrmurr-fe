    ...
    paymentsService.verifyPayment(reference)
      .then((data) => {
        if (data.status === "successful") {
          setStatus("success");
          setMessage("Your payment was successful! Your order is now being processed. Please check your email for order details.");
          success("Congratulations! Payment received and order is being processed.");
          setTimeout(() => {
            router.push("/");
          }, 3000);
        } else {
    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-[#f8f5f2] px-4">
      ...
      {status === "success" && (
        <div className="flex flex-col items-center">
          ...
          <p className="text-gray-500 text-sm">You will be redirected to the home page shortly.</p>
        </div>
      )}
      {status === "failed" && (
        <div className="flex flex-col items-center">
